...
def _apply_linkedin_filters(record: dict, filters: dict, mode: str) -> bool:
    ...
    if filters.get("industry"):
        val = (record.get("industry") or record.get("company_industry") or
               record.get("headline") or "").lower()
    ...
    if filters.get("management_tier"):
        val = (record.get("current_title") or record.get("title") or
               record.get("headline") or "").lower()
    ...
    if filters.get("email"):
        ...
        if isinstance(email_filter, bool):
            ...
            checks.append(domain in email_val.lower() if "@" in str(email_filter)
                          else bool(email_val))
    ...
    for i, record in enumerate(state["raw_records"]):
        try:
            ...
            title = (record.get("current_title") or record.get("title") or
                     record.get("headline") or "").strip()
            email = (record.get("email") or "").strip()

            print(f"[validate] {i+1}/{total} — {record.get('type','?')} | {name[:40]} | {title[:40]}")
            ...
            if not email:
                ...
                if website:
                    ...
                    slug  = record.get("url","").split("/in/")[-1].strip("/") or \
                            name.lower().replace(" ","-")
            ...
            messages = [
                ...
                ("human", json.dumps({
                    ...
                    "company":  record.get("current_company", record.get("company", "")),
            ...
            if not text:
                print(f"[validate] Empty Ollama response — keeping record with defaults")
                validated.append({**record, "intent_score": 0.5})
                continue
            # ──────────────────────────────────────────────────────────────
                ...
                validated.append({
                    ...
                    **result.get("enriched_fields", {}),
                    "intent_score": result.get(
                        "enriched_fields", {}).get("intent_score", 0.5)
                })
            ...
            print(f"[validate] JSON parse error on record {i+1}: {e} — keeping with defaults")
            validated.append({**record, "intent_score": 0.5})

...
import {
  Clock, RefreshCw, Mail, Eye, EyeOff,
  Users, AlertCircle, ChevronRight, Send
} from "lucide-react";
...
const TABS = [
  ...
  { key: "fu1",      label: "FU 1 Sent",    icon: Send,         statKey: "fu1" },
  { key: "fu2",      label: "FU 2 Sent",    icon: Send,         statKey: "fu2" },
  { key: "fu3plus",  label: "FU 3+",        icon: Send,         statKey: "fu3plus" },
    ...
    <div className="flex gap-1 items-center">
      {Array.from({length:max}).map((_,i)=>(
        <div key={i} className={`w-2 h-2 rounded-full transition-all
          ${status==="exhausted" ? "bg-gray-600" :
            i < current ? "bg-indigo-500" :
            i === current ? "bg-indigo-400/50" : "bg-gray-700"}`}/>
      ))}
...
function FollowUpCard({ seq }) {
  ...
  const isExhausted = seq.status === "exhausted";
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-gray-700 transition-all">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0
            ${isExhausted ? "bg-gray-800 text-gray-500" : "bg-gray-700 text-gray-300"}`}>
            {name[0]||"?"}
          </div>
          <div className="min-w-0">
            <p className={`font-medium text-sm truncate ${isExhausted ? "text-gray-500" : "text-white"}`}>
              {name}
            </p>
      ...
      {/* Step progress */}
      <div className="mb-3">
        <StepDots current={seq.current_step} max={seq.max_steps} status={seq.status}/>
      </div>

      {/* Status + follow-up count */}
      <div className="flex items-center gap-2 mb-3">
        {isExhausted ? (
          <span className="text-xs px-2 py-0.5 rounded-full bg-gray-800 text-gray-500 flex items-center gap-1">
            <AlertCircle size={9}/> Exhausted — no reply after {seq.max_steps} follow-ups
          </span>
        ) : seq.followups_sent > 0 ? (
          <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-900/30 text-indigo-400 border border-indigo-700/30 flex items-center gap-1">
            <Send size={9}/> {seq.followups_sent} follow-up{seq.followups_sent>1?"s":""} sent
          </span>
        ) : (
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          {!isExhausted && seq.next_followup_at ? (
            <div>
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Contacts who haven't replied yet · automated follow-up every 3 days · up to 5 attempts
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sequences.map(seq=>(
            <FollowUpCard key={seq.sequence_id} seq={seq}/>
          ))}
        </div>
      )}
    </div>

...
def load_thread_node(state: Agent4State) -> Agent4State:
    ...
    if seq:
        print(f"[agent4/load_thread] Existing sequence: status={seq['status']} asked_avail={seq['asked_availability']} has_teams={bool(seq['teams_meeting_url'])}")
    ...
    if seq:
        ...
        for row in cur.fetchall():
            history.append({"direction": row["direction"], "body": row["body"] or "",
                            "subject": row["subject"] or "", "ts": str(row["ts"]) if row["ts"] else ""})
